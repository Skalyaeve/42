...
long strToID(std::string &str, const bool& check)
{
	...
	// Check la syntaxe d'une IP
	if (check)
	{
		std::vector<std::string> values = ft::split(str, '.');
		if (values.size() != 4)
			return -1;
	...
	while (!stop && std::getline(config, line))
	{
		...
		while (!stopBis && !stream.fail() && (stream >> word))
		{
			...
			case (2):
				...
				while (!stream.fail())
				{
					const std::string codeStr(word.substr(0, word.find_first_of(".")));
					if (codeStr.find_first_not_of("0123456789") != std::string::npos)
						return configError("[ ERROR ] " + keywords[2] + ": " + lineIndex.str() + ERR_BAD_VAL, false, 0);
					
					const int code = std::atoi(codeStr.c_str());
					if (code > 999)
						return configError("[ ERROR ] " + keywords[2] + ": " + lineIndex.str() + ERR_BAD_VAL, false, 0);
					serv.errorPages[code] = word;
					stream >> word;
				}
				break;
			case (3):
