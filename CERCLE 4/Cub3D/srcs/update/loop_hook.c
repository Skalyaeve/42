...
void	find_wall(void)
{
	...
	if ((data())->side == 0)
		(data())->wall_dist = ((data())->ray_x - (data())->pos_x
				+ (1 - (data())->step_x) * 0.5) / (data())->ray_dir_x;
	else
		(data())->wall_dist = ((data())->ray_y - (data())->pos_y
				+ (1 - (data())->step_y) * 0.5) / (data())->ray_dir_y;
...
void	redraw_screen(void)
{
	while (++(data())->screen_x < (data())->screen_width)
	{
		init_values();
		find_wall();
		(data())->draw_start = -((data())->line_size * 0.5)
			+ ((data())->screen_height * 0.5);
		(data())->tex_y_offset = 0;
		if ((data())->draw_start < 0)
			(data())->tex_y_offset = (data())->draw_start;
		if ((data())->draw_start < 0)
			(data())->draw_start = 0;
		(data())->draw_end = ((data())->line_size * 0.5)
			+ ((data())->screen_height * 0.5) - 1;
